...
> **This copy lives in the stem repo** (the `.sh`/`.ks` it references are here).
> It was originally filed only in `krypton/handoffs/` — wrong place for stem-repo
> work, so L couldn't see it. The krypton 2.4.0 *release* handoff (linux/windows
> tarballs + brew formula bump) stays in `krypton/handoffs/handoff_m2lw_240_release.md`.
